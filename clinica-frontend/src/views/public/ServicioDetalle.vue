<template>
  <div class="overflow-hidden">
    ...
    <!-- CTA para contacto -->
    <section class="py-16 bg-gradient-to-br from-teal-50 to-white relative">
      <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-gradient-to-r from-teal-700 to-teal-600 rounded-xl p-8 md:p-12 text-white text-center shadow-lg">
          <h2 class="text-2xl md:text-3xl font-bold mb-4">¿Necesitas más información?</h2>
          <p class="text-lg mb-8 opacity-90">Contacta con nosotros para resolver tus dudas o concertar una cita</p>
          <router-link to="/contacto" class="inline-block px-8 py-3 bg-white text-teal-700 font-medium rounded-lg hover:bg-teal-50 transition-all duration-300 transform hover:scale-105 shadow-md">
            Contactar ahora
          </router-link>
        </div>
      </div>
...
const servicios = {
  ...
  // Más servicios podrían añadirse aquí...
};

// Servicio actual basado en el ID de la ruta
const servicioActual = computed(() => {
  // Si el servicio existe, lo devolvemos
  if (servicios[servicioId]) {
    return servicios[servicioId];
  }

  // Si no, devolvemos un servicio genérico
  return {
